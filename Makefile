...
GypAndroid.mk: ./deps/gyp example/libtextsort.gyp support-lib/support_lib.gyp example/example.djinni
	...
	ANDROID_BUILD_TOP=$(shell dirname `which ndk-build`) deps/gyp/gyp --depth=. -f "gyp_extensions/android.py" -DOS=android -Icommon.gypi example/libtextsort.gyp --root-target=libtextsort_jni
